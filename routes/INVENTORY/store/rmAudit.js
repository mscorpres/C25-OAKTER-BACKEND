    ...
    if (stmt3.length > 0) {
      ...
      return res.json({
        ...
        data: {
          ...
          available_qty: helper.number(helper.number(inward_all_qty) - helper.number(outward_all_qty)),
        },
      });
    } else {
      return res.json({
...
// CREATE FGOUT
router.post("/saveAudit", [auth.isAuthorized, auth.checkDuplicacy_db], async (req, res) => {
  ...
  try {
    let stmt = await invtDB.query("SELECT `audit_ref_id` FROM `ims_rm_audit` GROUP BY `audit_ref_id` ORDER BY ID DESC LIMIT 1", {
      type: invtDB.QueryTypes.SELECT,
    });
    let transactionCode;
...
router.post("/updateAudit", [auth.isAuthorized], async (req, res) => {
  ...
  try {
    ...
    const stmt_status = await invtDB.query("SELECT * FROM ims_rm_audit_log WHERE ID = :audit_key AND component_key = :component_key ", {
      replacements: {
        audit_key: audit_key,
...
router.post("/updateRejectedAudit", [auth.isAuthorized], async (req, res) => {
  ...
  try {
    ...
    const stmt_status = await invtDB.query("SELECT * FROM ims_rm_audit_log WHERE ID = :audit_key AND component_key = :component_key ORDER BY ID DESC LIMIT 1 ", {
      replacements: {
        audit_key: audit_key,
        component_key: req.body.component_key,
      },
