...
router.post("/fetchProductData", [auth.isAuthorized], async (req, res) => {
  try {
    const [product] = await invtDB.query(
      `SELECT products.*, units.units_name
       FROM products
       LEFT JOIN units ON products.p_uom = units.units_id
       WHERE products.product_key = :key
       ORDER BY products.p_name ASC`,
      {
    ...
    const sku = product.p_sku;
    const location = req.body.location || req.body.locations;

    // Location given -> stock AT that location (TRANSFER counted as IN there,
    // and the matching TRANSFER's pick-side OUT row must still count as OUT
    // there, so fg_out_type is NOT filtered here).
    // Location not given -> company-wide stock, same formula as report/q3.js
    // closingqty (TRANSFER counted as IN, its pick-side OUT excluded via
    // fg_out_type != '--' so a transfer doesn't change the overall total).
    const [inResult, outResult] = await Promise.all([
      // CREDIT (TOTAL IN)
      invtDB.query(
        `SELECT COALESCE(SUM(mfg_approve_in_qty), 0) AS totalQTYin
         FROM mfg_production_3
         WHERE company_branch = :branch
           AND mfg_pro_apr_sku = :sku
           AND type IN ('IN', 'FGMIN', 'TRANSFER')
           AND fg_status = 'ACTIVE'
           ${location ? "AND mfg_pro_location_in = :location" : ""}`,
        {
          replacements: { sku, branch, location },
          type: invtDB.QueryTypes.SELECT,
        },
      ),
      ...
      // DEBIT (TOTAL OUT)
      invtDB.query(
        `SELECT COALESCE(SUM(fgout_approve_out_qty), 0) AS totalQTYout
         FROM mfg_production_3
         WHERE company_branch = :branch
           AND fgout_pro_apr_sku = :sku
           AND type = 'OUT'
           AND fg_status = 'ACTIVE'
           ${location ? "AND fgout_pro_location_out = :location" : "AND fg_out_type != '--'"}`,
        {
          replacements: { sku: product.product_key, branch, location },
          type: invtDB.QueryTypes.SELECT,
        },
      ),
    ]);

    const totalIn = parseFloat(inResult[0]?.totalQTYin ?? 0);
    const totalOut = parseFloat(outResult[0]?.totalQTYout ?? 0);
    const war = await lastFGWeightedAverageRate(sku);
    const stock = totalIn - totalOut < 0 ? 0 : totalIn - totalOut;
  ...
  async (req, res) => {
    ...
    if (dubliEle.length > 0) {
      res.json({
        success: false,
        message:
          "You have entered a same product twice of time in a single request",
        status: "error",
      });
      return;
    }
            ...
            "INSERT INTO mfg_production_3 (txn_session,company_branch,fgout_pro_apr_sku,fgout_approve_out_qty,fgout_pro_apr_by,fgout_pro_apr_date,fgout_pro_apr_fulldate, fgout_pro_location_out,mfg_pro_FGout_transaction,type,fg_out_type,fg_out_remark,in_fg_rate)VALUES (:txn_session,:branch,:sku,:aproutqty,:outby,:outdate,:outfulldate, :fgout_pro_location_out,:transactioncode,:type, :fg_out_type,:remark, :rate)",
            {
              replacements: {
                ...
                outdate: moment(new Date())
                  .tz("Asia/Kolkata")
                  .format("YYYY-MM-DD"),
                outfulldate: moment(new Date())
                  .tz("Asia/Kolkata")
                  .format("YYYY-MM-DD HH:mm:ss"),
                fgout_pro_location_out:
                  req.body.location[i] == null ? "--" : req.body.location[i],
          ...
          if (stmt1.length > 0) {
            ...
            if (stmt2.length > 0) {
              let stmt3 = await invtDB.query(
                "SELECT p_sku, product_key FROM products WHERE product_key = :key",
                {
              ...
              if (stmt3.length > 0) {
                const lineLocation =
                  req.body.location[i] == null ? "--" : req.body.location[i];
                //TOTAL IN YET (same credit formula as fetchProductData / report q3.js)
                let stmt4 = await invtDB.query(
                  "SELECT mfg_pro_apr_sku,type,entry_mode, COALESCE(SUM(mfg_approve_in_qty),0) AS totalYetInstock FROM mfg_production_3 WHERE mfg_pro_apr_sku= :sku AND type IN('IN', 'FGMIN', 'TRANSFER') AND company_branch = :branch AND fg_status = 'ACTIVE' AND mfg_pro_location_in = :location",
                  {
                    replacements: { sku: stmt3[0].p_sku, branch: req.branch, location: lineLocation },
                    type: invtDB.QueryTypes.SELECT,
                  },
                );
                ...
                //TOTAL OUT YET (same debit formula as fetchProductData / report q3.js)
                let stmt5 = await invtDB.query(
                  "SELECT COALESCE(SUM(fgout_approve_out_qty),0) AS totalYetOutstock FROM mfg_production_3 WHERE fgout_pro_apr_sku = :sku AND type = 'OUT' AND company_branch = :branch AND fg_status = 'ACTIVE' AND fgout_pro_location_out = :location",
                  {
                    replacements: {
                      sku: stmt3[0].product_key,
                      branch: req.branch,
                ...
                if (
                  helper.number(totalYetInstock - totalYetOutstock) >=
                  ...
                  if (i == product_length - 1) {
                    ...
                    return res.json({
                      success: true,
                      message: "FG OUT Completed..\nTxnID: #" + transactionCode,
                      status: "success",
                    });
                  }
                } else {
                  ...
                  return res.json({
                    success: false,
                    message: "getting an insufficient stock for out",
                    status: "error",
                  });
                }
              } else {
                ...
                return res.json({
                  success: false,
                  messge: "some of the SKU(s) are not valid",
                  status: "error",
                });
              }
            } else {
              ...
              return res.json({
                success: false,
                message: "an error while executing your request(2)",
                status: "error",
              });
            }
          } else {
            ...
            return res.json({
              success: false,
              message: "an error while executing your request (1)",
              status: "error",
            });
          }
        }
