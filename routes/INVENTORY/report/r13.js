...
router.get("/", [auth.isAuthorized], async (req, res) => {
	try {
		let db_stmt = await otherDB.query("SELECT * FROM invt_r10 WHERE user_id = 'CRN103522'", { type: invtDB.QueryTypes.SELECT });
		if (db_stmt.length > 0) {
...
router.post("/update", [auth.isAuthorized], async (req, res) => {
	...
	try {
		let stmt = await otherDB.query("UPDATE `invt_r10` SET `parts` = :parts, `locations` = :locs WHERE `invt_r10`.`ID` = 1;", {
			replacements: {
				parts: parts,
				locs: locations,
			},
...
// GET SELECTED VALUES
router.get("/getSelectedValue", [auth.isAuthorized], async (req, res) => {
	try {
		let stmt = await otherDB.query("SELECT * FROM `invt_r10`", { type: otherDB.QueryTypes.SELECT });
