...
router.post("/createCreditVoucher", [auth.isAuthorized], async (req, res) => {
    ...
    try {
        ...
        // NUMBURING FUN
        let stmt_number = await tallyDB.query("SELECT * FROM `tally_numbering` WHERE `for_number` = 'CN'", {
        ...
        if (stmt_number.length > 0) {
            ...
            debit_no = "CN01/" + currYear + "-" + (currYear + 1) + "/0001";
        }
        // END NUMBURING FUN

        await tallyDB.query("UPDATE `tally_numbering` SET `suffix` = `suffix`+1 WHERE `for_number`= 'CN'", {
...
// fetch credit voucher list
router.post("/creditVoucherList", [auth.isAuthorized], async (req, res) => {
    ...
    try {
        ...
        if (wise == "eff_wise") {
            ...
            main_stmt = await tallyDB.query(
                "SELECT `module_used`, `ref_date`, `which_module`, `ledger_data_status` as status, `ledegr`.`ladger_name` as account,`ledegr`.`code` as account_code, `tally_ledger_data`.`debit`, `tally_ledger_data`.`credit`, `tally_ledger_data`.`comment` FROM `tally_ledger_data` LEFT JOIN `tally_ledger` AS ledegr ON `ledegr`.`ledger_key` = `tally_ledger_data`.`ladger_key` WHERE (DATE_FORMAT(`tally_ledger_data`.`ref_date`,'%Y-%m-%d') BETWEEN :date1 AND :date2) AND `which_module` = 'CN'",
                {
                    replacements: { date1: date1, date2: date2 },
                    type: tallyDB.QueryTypes.SELECT,
                }
        ...
        if (wise == "code_wise") {
            main_stmt = await tallyDB.query(
                "SELECT `module_used`, `ref_date`, `which_module`, `ledger_data_status` as status, `ledegr`.`ladger_name` as account,`ledegr`.`code` as account_code, `tally_ledger_data`.`debit`, `tally_ledger_data`.`credit`, `tally_ledger_data`.`comment` FROM `tally_ledger_data` LEFT JOIN `tally_ledger` AS ledegr ON `ledegr`.`ledger_key` = `tally_ledger_data`.`ladger_key`  WHERE `module_used` = :data  AND `which_module` = 'CN'",
                {
        ...
        if (wise == "vendor_wise") {
            main_stmt = await tallyDB.query(
                "SELECT `module_used`, `ref_date`, `which_module`, `ledger_data_status` as status, `ledegr`.`ladger_name` as account,`ledegr`.`code` as account_code, `tally_ledger_data`.`debit`, `tally_ledger_data`.`credit`, `tally_ledger_data`.`comment` FROM `tally_ledger_data` LEFT JOIN `tally_ledger` AS ledegr ON `ledegr`.`ledger_key` = `tally_ledger_data`.`ladger_key`  WHERE tally_ledger_data.ladger_key = :data  AND `which_module` = 'CN'",
                {
                    replacements: { data: data },
                    type: tallyDB.QueryTypes.SELECT,
                }
...
// FETCH CREDIT VOUCHER DETAILS
router.post("/creditVoucherDetail", [auth.isAuthorized], async (req, res) => {
    let validation = new Validator(req.body, {
        cn_key: "required",
    });
    if (validation.fails()) {
...
router.post("/printCreditVoucher", [auth.isAuthorized], async (req, res) => {
    let validation = new Validator(req.body, {
        cn_key: "required",
    });
    if (validation.fails()) {
    ...
    try {
        let stmt = await tallyDB.query("SELECT  `tally_ledger`.`ladger_name`,`tally_ledger`.`code`,debit,credit,ref_date,module_used FROM `tally_ledger_data` LEFT JOIN `tally_ledger` ON  `tally_ledger_data`.`ladger_key`=`tally_ledger`.`ledger_key` WHERE `module_used` = :data AND `which_module` = 'CN'", {
            replacements: { data: req.body.cn_key },
            type: tallyDB.QueryTypes.SELECT,
        });
        if (stmt.length > 0) {
            let data = {
                cn_code: stmt[0].module_used,
        ...
        `;
            ...
            let file = { content: require("./printHtml/creditVoucherHtml").printHtml(data, rows, row_total) };
...
// EDIT CREDIT VOUCHER
router.post("/editCreditVoucher", [auth.isAuthorized], async (req, res) => {
    let validation = new Validator(req.body, {
        cn_key: "required",
    });
    if (validation.fails()) {
...
router.post("/updateCreditVoucher", [auth.isAuthorized], async (req, res) => {
    let validation = new Validator(req.body, {
        cn_key: "required",
        effective_date: "required",
    });
    ...
    try {
        ...
        await transaction.commit();
        res.json({ status: "success", success: true, message: "Credit Voucher updated successfully" });
    } catch (err) {
        return helper.errorResponse(res, err);
    }
});
