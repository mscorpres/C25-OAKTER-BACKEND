...
// FETCH VBT02
router.post("/fetch_vbt02", [auth.isAuthorized], async (req, res) => {
  ...
  try {
    ...
    if (wise == "date_wise") {
      ...
      main_stmt = await invtDB.query(
        "SELECT `rm_location`.`insert_date` AS `min_in_date`, `rm_location`.`in_transaction_id` AS `min_transaction`, `components`.`c_part_no` AS `part_code`, `rm_location`.`in_vendor_name` AS `ven_code` FROM `rm_location` LEFT JOIN `components` ON rm_location.components_id=components.component_key WHERE `components`.`c_type`='S' AND `trans_type`='INWARD' AND `in_vendor_name` = :ven AND (`rm_location`.`vendor_type` = 'v01' OR `rm_location`.`vendor_type` = 'j01') AND `vbp_status`='N' ORDER BY rm_location.ID",
        {
...
// ADD VBT02
router.post("/add_vbt02", [auth.isAuthorized], async (req, res) => {
  let validation = new Validator(req.body, {
    ven_code: "required",
  ...
  try {
    ...
    const bill_amount = Number(req.body.bill_amount).toFixed(2);
    const ven_ammount = Number(total_ven_ammount).toFixed(2);

    if (Math.abs(bill_amount - ven_ammount) != 0) {
      return res.json({ status: "error", success: false, message: "Bill amount ${bill_amount} and Vendor amount ${ven_ammount} not match " });
    }
    ...
    for (let i = 0; i < comp_length; i++) {
      // if (Number(req.body.bill_qty[i]) < Number(req.body.in_qtys[i])) {
      //   await transaction.rollback();
      //   await transactionInvt.rollback();
      //   return res.json({ status: "error", success: false, message: "Bill QTY ${Number(req.body.bill_qty[i])} >= QTY ${Number(req.body.in_qtys[i])} " });
      // }
      ...
      let stmt = await tallyDB.query(
        "INSERT INTO `tally_vbt` ( `part_code`, po_number, project_id, `vbt_inqty`,`vbt_bill_qty`, `vbt_inrate`, `vbt_taxable_value`, `hsn_code`, `vbt_gst_type`, `vbt_gst_rate`, `vbp_gst_ass_value`, `vbt_cgst`,`vbt_cgst_gl`, `vbt_sgst`,`vbt_sgst_gl`, `vbt_igst`,`vbt_igst_gl`, `gl_code`, `tds_code`,`tds_gl`, `vbt_ven_ammount`, `vbt_key`, `insert_by`, `insert_date`, `min_id` , inrPrice , cifPrice , cifValue ,`vbt_tds_ass_val`,`vbt_tds_amount`, `ven_address`, `vbt_invoice_no`, `vbt_invoice_date`, `vbt_comment`,`ven_code`, `vbt_gstin`,`vbt_type` , `effective_date` , `item_description` , billAmount ) VALUES (:part_code,  :po_number, :project_id, :in_qtys, :vbt_bill_qty, :in_rates, :taxable_values, :hsn_code, :in_gst_types, :vbt_gst_rate, :gst_ass_vals, :cgsts,  :cgsts_gl, :sgsts, :sgsts_gl, :igsts, :igsts_gl, :g_l_codes, :tds_codes, :tds_gl, :ven_amounts, :vbt_key, :insert_by, :insert_date, :min_id , :inrPrice, :cifPrice, :cifValue, :tds_ass_vals, :tds_amounts,:ven_address, :invoice_no, :invoice_date, :comment, :ven_code, :vbt_gstin, 'VBT02' , :effective_date , :item_description , :billAmount )",
        {
          replacements: {
...
// VBT02 GL GROUP OPTION
router.get("/vbt02_gl_options", [auth.isAuthorized], async (req, res) => {
  try {
    let stmt = await tallyDB.query("SELECT `vbt_group_key` FROM `vbt_module` WHERE `vbt_module`='vbt02'", {
      type: tallyDB.QueryTypes.SELECT,
    });
    if (stmt.length > 0) {
...
router.put("/update", [auth.isAuthorized], async (req, res) => {
  ...
  try {
    ...
    let deleteVbt = await tallyDB.query("DELETE FROM tally_vbt WHERE vbt_key = :vbtKey AND vbt_type = 'VBT02' AND vbt_debit_key = '--'", {
      replacements: { vbtKey: vbt_key },
      type: tallyDB.QueryTypes.DELETE,
      transaction: transaction,
    });

    let deleteTallyLedgerData = await tallyDB.query("DELETE FROM tally_ledger_data WHERE module_used = :vbtKey AND which_module = 'VBT02' AND debit_key = '--'", {
      replacements: { vbtKey: vbt_key },
      type: tallyDB.QueryTypes.DELETE,
      transaction: transaction,
    });
    ...
    for (let i = 0; i < comp_length; i++) {
      ...
      let stmt = await tallyDB.query(
        "INSERT INTO `tally_vbt` ( `part_code`, po_number, project_id, `vbt_inqty`,`vbt_bill_qty`, `vbt_inrate`, `vbt_taxable_value`, `hsn_code`, `vbt_gst_type`, `vbt_gst_rate`, `vbp_gst_ass_value`, `vbt_cgst`,`vbt_cgst_gl`, `vbt_sgst`,`vbt_sgst_gl`, `vbt_igst`,`vbt_igst_gl`, `gl_code`, `tds_code`,`tds_gl`, `vbt_ven_ammount`, `vbt_key`, `insert_by`, `insert_date`, `min_id` , inrPrice , cifPrice , cifValue ,`vbt_tds_ass_val`,`vbt_tds_amount`, `ven_address`, `vbt_invoice_no`, `vbt_invoice_date`, `vbt_comment`,`ven_code`, `vbt_gstin`,`vbt_type` , `effective_date` , `item_description` , update_date , update_by , billAmount ) VALUES (:part_code,  :po_number, :project_id, :in_qtys, :vbt_bill_qty, :in_rates, :taxable_values, :hsn_code, :in_gst_types, :vbt_gst_rate, :gst_ass_vals, :cgsts,  :cgsts_gl, :sgsts, :sgsts_gl, :igsts, :igsts_gl, :g_l_codes, :tds_codes, :tds_gl, :ven_amounts, :vbt_key, :insert_by, :insert_date, :min_id , :inrPrice, :cifPrice, :cifValue, :tds_ass_vals, :tds_amounts,:ven_address, :invoice_no, :invoice_date, :comment, :ven_code, :vbt_gstin, 'VBT02' , :effective_date , :item_description , :update_date , :update_by , :billAmount )",
        {
          replacements: {
            ...
            tds_ass_vals: req.body.tds_ass_vals?.[i] ? req.body.tds_ass_vals[i] : "",
