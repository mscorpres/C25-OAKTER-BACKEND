...
router.post("/fetch_vbt08", async (req, res) => {
      ...
      if (item.type === "FG") {
        main_stmt = await invtDB.query(
          ...
          WHERE mfg_production_3.mfg_pro_apr_transaction = :min_id
            ...
            AND mfg_production_3.inward_type = 'SALES-RETURN'
            AND mfg_production_3.vendor_type = 's01'
...
// ADD VBT08
router.post("/add_vbt08", [auth.isAuthorized], async (req, res) => {
  let validation = new Validator(req.body, {
    ven_code: "required",
  ...
  try {
    ...
    // NUMBURING FUN
    let stmt_number = await tallyDB.query("SELECT * FROM `tally_numbering` WHERE `for_number` = 'VBT08' FOR UPDATE", {
      type: tallyDB.QueryTypes.SELECT,
    ...
    if (stmt_number.length > 0) {
      ...
      vbt_no = "VBT08/" + currYear + "-" + (currYear + 1) + "/0001";
    }

    await tallyDB.query("UPDATE `tally_numbering` SET `suffix` = `suffix`+1 WHERE `for_number`= 'VBT08'", {
...
router.get("/vbt08_gl_options", async (req, res) => {
  try {
    let stmt = await tallyDB.query("SELECT `vbt_group_key` FROM `vbt_module` WHERE `vbt_module`='vbt08'", {
      type: tallyDB.QueryTypes.SELECT,
    });
    if (stmt.length > 0) {
