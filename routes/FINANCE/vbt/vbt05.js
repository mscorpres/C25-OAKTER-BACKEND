...
// VBT05 // EXPENSES
// FETCH VBT
router.post("/fetch_vbt05", [auth.isAuthorized], async (req, res) => {
...
// ADD VBT05
router.post("/add_vbt05", [auth.isAuthorized], async (req, res) => {
  let validation = new Validator(req.body, {
    ven_code: "required",
  ...
  try {
    ...
    // NUMBURING FUN
    let stmt_number = await tallyDB.query("SELECT * FROM `tally_numbering` WHERE `for_number` = 'VBT05' FOR UPDATE", {
      type: tallyDB.QueryTypes.SELECT,
    ...
    if (stmt_number.length > 0) {
      ...
      vbt_no = "VBT05/" + currYear + "-" + (currYear + 1) + "/0001";
    }

    await tallyDB.query("UPDATE `tally_numbering` SET `suffix` = `suffix`+1 WHERE `for_number`= 'VBT05'", {
...
// VBT05 GL GROUP OPTION
router.get("/vbt05_gl_options", [auth.isAuthorized], async (req, res) => {
  try {
    let stmt = await tallyDB.query("SELECT `vbt_group_key` FROM `vbt_module` WHERE `vbt_module`='vbt05'", {
      type: tallyDB.QueryTypes.SELECT,
    });
    if (stmt.length > 0) {
  ...
  try {
    ...
    if (stmt.length > 0) {
      let stmt1 = await tallyDB.query(
        "UPDATE `tally_ledger_data` SET `ledger_data_status` = 'D',`deleted_by` = :deleted_by ,`deleted_date` = :deleted_date WHERE `tally_ledger_data`.`module_used` = :vbt_code",
        {
          replacements: {
            vbt_code: req.body.vbt_code,
            deleted_by: req.logedINUser,
...
// DEBIT VBT05
router.post("/debit_vbt05", [auth.isAuthorized], async (req, res) => {
  let validation = new Validator(req.body, {
    vbt_code: "required",
    ven_code: "required",
    ven_address: "required",
      ...
      let stmt = await tallyDB.query(
        "INSERT INTO `tally_vbt` ( `part_code`, po_number, project_id, `vbt_bill_qty`, `vbt_inrate`, `vbt_taxable_value`, `hsn_code`, `vbt_gst_type`, `vbt_gst_rate`, `freight`, `vbt_freight_gl`, `vbp_gst_ass_value`, `vbt_cgst`,`vbt_cgst_gl`, `vbt_sgst`,`vbt_sgst_gl`, `vbt_igst`,`vbt_igst_gl`, `gl_code`, `tds_code`, `tds_gl`, `vbt_ven_ammount`, `vbt_key`, `vbt_debit_key` , `insert_by`, `insert_date`, `min_id`,`vbt_tds_ass_val`,`vbt_tds_amount`, `ven_address`, `vbt_invoice_no`, `vbt_invoice_date`, `vbt_comment`,`ven_code`, `vbt_gstin`,`vbt_type` , `effective_date` , `vbt_status`) VALUES (:part_code,  :po_number, :project_id, :vbt_bill_qty, :in_rates, :taxable_values, :hsn_code, :in_gst_types, :vbt_gst_rate, :freight, :freight_gl, :gst_ass_vals, :cgsts, :cgsts_gl, :sgsts, :sgsts_gl, :igsts, :igsts_gl, :g_l_codes, :tds_codes, :tds_gl, :ven_amounts, :vbt_key, :vbt_debit_key , :insert_by, :insert_date, :min_id, :tds_ass_vals, :tds_amounts, :ven_address, :invoice_no, :invoice_date, :comment, :ven_code, :vbt_gstin, 'VBT05' , :effective_date, :vbt_status )",
        {
          replacements: {
            ...
            hsn_code: req.body.hsn_code[i],
            in_gst_types: req.body.in_gst_types[i],
            freight: req.body.freight_gl[i], //req.body.freight[i],
            freight_gl: "TP550175734290",
            gst_ass_vals: req.body.gst_ass_vals[i],
            cgsts: req.body.cgsts[i],
...
router.put("/update", [auth.isAuthorized], async (req, res) => {
  ...
  try {
    ...
    total_credit = Math.abs(Number(Number(total_credit).toFixed(2)));
    total_debit = Number(Number(total_debit).toFixed(2));
    ...
    let deleteVbt = await tallyDB.query("DELETE FROM tally_vbt WHERE vbt_key = :vbtKey AND vbt_type = 'VBT05' AND vbt_debit_key = '--'", {
      replacements: { vbtKey: vbt_key },
      type: tallyDB.QueryTypes.DELETE,
      transaction: transaction,
    });

    let deleteTallyLedgerData = await tallyDB.query("DELETE FROM tally_ledger_data WHERE module_used = :vbtKey AND which_module = 'VBT05' AND debit_key = '--'", {
      replacements: { vbtKey: vbt_key },
      type: tallyDB.QueryTypes.DELETE,
      transaction: transaction,
    });
    ...
    for (let i = 0; i < comp_length; i++) {
      ...
      let stmt = await tallyDB.query(
        "INSERT INTO `tally_vbt` ( `part_code`,po_number, project_id, `vbt_inqty`,`vbt_bill_qty`, `vbt_inrate`, `vbt_taxable_value`, `hsn_code`, `vbt_gst_type`, `vbt_gst_rate`, `freight`, `vbt_freight_gl`, `vbp_gst_ass_value`, `vbt_cgst`,`vbt_cgst_gl`, `vbt_sgst`,`vbt_sgst_gl`, `vbt_igst`,`vbt_igst_gl`, `gl_code`, `tds_code`, `tds_gl`, `vbt_ven_ammount`, `vbt_key`, `insert_by`, `insert_date`, `min_id` , inrPrice , cifPrice , cifValue ,`vbt_tds_ass_val`,`vbt_tds_amount`, `ven_address`, `vbt_invoice_no`, `vbt_invoice_date`, `vbt_comment`,`ven_code`, `vbt_gstin`,`vbt_type` , `effective_date` , `item_description` , update_date , update_by , billAmount ) VALUES (:part_code,:po_number, :project_id, :in_qtys, :vbt_bill_qty, :in_rates, :taxable_values, :hsn_code, :in_gst_types, :vbt_gst_rate,  :freight, :freight_gl, :gst_ass_vals, :cgsts, :cgsts_gl, :sgsts, :sgsts_gl, :igsts, :igsts_gl, :g_l_codes, :tds_codes, :tds_gl, :ven_amounts, :vbt_key, :insert_by, :insert_date, :min_id , :inrPrice, :cifPrice, :cifValue, :tds_ass_vals, :tds_amounts,:ven_address, :invoice_no, :invoice_date, :comment, :ven_code, :vbt_gstin, 'VBT05' , :effective_date , :item_description , :update_date , :update_by , :billAmount)",
        {
          replacements: {
